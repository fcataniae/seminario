...
public interface LocalRepository extends JpaRepository<Local, Long> {
    ...
    @Query(value = "SELECT IFNULL(aux2.TIENDA,aux.tienda), IFNULL(CAST(aux.CANT_RECIBIDA as SIGNED),0) as CANT_RECIBIDA, IFNULL(CAST(aux2.CANT_ENVIADA as SIGNED),0) as CANT_ENVIADA FROM ( \n " +
            ...
            "UNION \n " +
            "SELECT IFNULL(aux2.TIENDA,aux.tienda), IFNULL(CAST(aux.CANT_RECIBIDA as SIGNED),0) as CANT_RECIBIDA , IFNULL(CAST(aux2.CANT_ENVIADA as SIGNED),0) as CANT_ENVIADA FROM ( \n " +
            "\t SELECT m.DESTINO AS TIENDA, SUM(im.CANTIDAD) AS CANT_RECIBIDA  \n " +
            "\t FROM seminario.movimiento m \n " +
            "\t LEFT JOIN seminario.itemmovimiento im on m.id = im.movimiento_id  \n " +
            "\t WHERE m.id_tipo_movimiento = 1 -- ENVIO \n " +
